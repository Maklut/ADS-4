...
int main() {
  ...
  for (int size = 100; size <= 2000; size += 100) {
    ...
    // Измеряем время для countPairs1
    ...
    // Измеряем время для countPairs2
    ...
    // Измеряем время для countPairs3
    start = std::chrono::high_resolution_clock::now();
  ...
  if (result == 0) {
    ...
    std::cout << "You can plot graphs manually using:\n";
    std::cout << "  - Excel\n";
    std::cout << "  - Python with matplotlib\n";
    std::cout << "  - Any other plotting tool\n";
  }
