...
long long measureTime(int (*func)(int*, int, int), int* arr, int len, int value) {
...
int main() {
    std::cout << "Исследование зависимости времени выполнения от размера массива\n";
    std::cout << "============================================================\n\n";
    ...
    std::cout << "Результаты " << ((result1 == result2 && result2 == result3) ? "совпадают \n" : "НЕ совпадают \n");
    ...
    std::vector<long long> times1, times2, times3;
    
    std::cout << "\nВыполнение измерений...\n";
    std::cout << "Размер\t\tВремя1 (мкс)\tВремя2 (мкс)\tВремя3 (мкс)\n";
    std::cout << "--------------------------------------------------------\n";
    ...
    for (int size : sizes) {
        ...
        long long t1 = measureTime(countPairs1, arr, size, value);
        long long t2 = measureTime(countPairs2, arr, size, value);
        long long t3 = measureTime(countPairs3, arr, size, value);
        
        times1.push_back(t1);
        times2.push_back(t2);
        times3.push_back(t3);
        
        std::cout << size << "\t\t" << t1 << "\t\t" << t2 << "\t\t" << t3 << "\n";
    ...
    std::cout << "\nДля построения графиков используйте Python скрипт plot_graphs.py\n";
